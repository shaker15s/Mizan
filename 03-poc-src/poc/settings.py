...
_SPECS: tuple[SettingSpec, ...] = (
    ...
    # Additive signal layer only: nothing here can authorize, execute, approve or
    # verify an ERP action. ``decision.provider=off`` (the default) keeps the
    # un-integrated baseline, and selecting a provider is always explicit.
    _s("decision.provider", "decision", "مزوّد إشارة القرار", "off", type=TYPE_CHOICE,
       choices=("off", "mock", "typesafe", "custom"), env_var="DECISION_PROVIDER",
       help_ar="off = بدون إشارة قرار (الأساس). mock = محلي بدون شبكة. typesafe = Jev الرسمي. custom = endpoint بمواصفة System One."),
    _s("decision.mode", "decision", "وضع تأثير الإشارة", "off", type=TYPE_CHOICE,
       choices=("off", "shadow", "advisory", "enforcing"), env_var="DECISION_MODE",
       help_ar="shadow = تسجيل بدون تأثير. advisory = قد تضيّق قائمة الأدوات أو تطلب توضيح. enforcing = بعد اجتياز بوابات الإصدار فقط."),
    _s("decision.model", "decision", "معرّف نموذج القرار", "jev-latest", env_var="DECISION_MODEL",
       help_ar="ثبّت نسخة صريحة (jev-1.13.0) لو ضبطت العتبات على سلوكها."),
    _s("decision.base_url", "decision", "عنوان مزوّد القرار", "", env_var="DECISION_BASE_URL",
       help_ar="افتراضيًا https://api.typesafe.ai للمزوّد الرسمي."),
    _s("decision.path", "decision", "مسار التقييم", "/v1/systemone", env_var="DECISION_PATH"),
    _s("decision.api_key", "decision", "مفتاح مزوّد القرار", "", type=TYPE_SECRET, env_var="TYPESAFE_API_KEY",
       help_ar="يُخزَّن في ذاكرة العملية فقط، ولا يُكتب على القرص ولا يُرسل لأي مزوّد آخر ولا يظهر في الواجهة."),
    _s("decision.timeout_seconds", "decision", "مهلة استدعاء القرار (ثانية)", 10.0, type=TYPE_FLOAT, min_value=1.0, max_value=60.0),
    _s("decision.retries", "decision", "إعادة محاولة القرار", 1, type=TYPE_INT, min_value=0, max_value=5,
       help_ar="إعادة المحاولة للقرار فقط — لا تعيد تنفيذ أي عملية ERP أبدًا."),
    _s("decision.threshold_version", "decision", "إصدار العتبات", "1.1.0", env_var="DECISION_THRESHOLD_VERSION"),
    _s("decision.route_min_confidence", "decision", "أدنى ثقة لاختيار الأداة", 0.85, type=TYPE_FLOAT, min_value=0.0, max_value=1.0,
       help_ar="مُعايَرة على مجموعة المعايرة بمزوّد اصطناعي (v1.1.0)؛ أعِد المعايرة على مزوّد حقيقي قبل الاعتماد."),
    _s("decision.route_min_margin", "decision", "أدنى فارق بين الاحتمالَين الأول والثاني", 0.50, type=TYPE_FLOAT, min_value=0.0, max_value=1.0),
    _s("decision.ambiguity_trigger", "decision", "عتبة الإبهام", 0.60, type=TYPE_FLOAT, min_value=0.0, max_value=1.0),
    _s("decision.injection_quarantine", "decision", "عتبة حجر حقن التعليمات", 0.90, type=TYPE_FLOAT, min_value=0.0, max_value=1.0),
    _s("decision.injection_flag", "decision", "عتبة الإشارة للحقن", 0.70, type=TYPE_FLOAT, min_value=0.0, max_value=1.0),
    _s("decision.risk_escalate_level", "decision", "عتبة المخاطر الدلالية", 3.0, type=TYPE_FLOAT, min_value=0.0, max_value=4.0,
       help_ar="مستوى في سلّم المخاطر الدلالية (0..4). لا يقلّل أبدًا مستوى المخاطر الحتمي."),
    _s("decision.review_claim_mismatch", "decision", "عتبة عدم تطابق الادعاء", 0.60, type=TYPE_FLOAT, min_value=0.0, max_value=1.0),
    _s("decision.review_anomaly", "decision", "عتبة الشذوذ في التتبّع", 0.80, type=TYPE_FLOAT, min_value=0.0, max_value=1.0),
    _s("decision.narrow_tool_set", "decision", "تضييق قائمة الأدوات (استشاري)", True, type=TYPE_BOOL,
       help_ar="في وضع advisory فقط: تقليل الأدوات المعروضة للنموذج، مع إبقاء بديل ثانٍ دائمًا."),
    _s("decision.keep_runner_up_tool", "decision", "إبقاء الأداة الثانية", True, type=TYPE_BOOL),
    _s("decision.cache_read_routes", "decision", "كاش قرارات القراءة", False, type=TYPE_BOOL,
       help_ar="لا يُخزَّن أبدًا قرار كتابة أو تفويض. المفتاح يشمل hash الحالة وإصدارات المواصفة/النموذج/العتبات."),
    _s("decision.cache_ttl_seconds", "decision", "عمر كاش القرار (ثانية)", 300, type=TYPE_INT, min_value=30, max_value=3600),
    _s("decision.post_run_review", "decision", "فحص دلالي بعد التنفيذ", False, type=TYPE_BOOL,
       help_ar="إشارة مراجعة فقط؛ التحقق الحتمي هو مصدر الحقيقة ولا يتغير."),
    _s("decision.escalate_risk_level", "decision", "السماح برفع مستوى المخاطر", False, type=TYPE_BOOL,
       help_ar="متوقف افتراضيًا: يسمح للإشارة الدلالية برفع المستوى (تصعيد فقط، لا خفض أبدًا) وهو مطلوب فقط في وضع enforcing."),
    _s("decision.allow_external_provider", "decision", "السماح بمزوّد خارجي (غير رسمي)", False, type=TYPE_BOOL,
       help_ar="مطلوب لأي endpoint غير رسمي. استخدم بيانات اصطناعية فقط في هذا الوضع."),
    _s("decision.unofficial_provider", "decision", "وسم المزوّد كغير رسمي", False, type=TYPE_BOOL,
       help_ar="عند التشغيل: كل سجل وواجهة تُظهر أن المزوّد غير رسمي وغير مضمون."),
    _s("decision.synthetic_only", "decision", "بيانات اصطناعية فقط", True, type=TYPE_BOOL,
       help_ar="يبقى مُشغَّلًا للتطوير المجاني: لا تُرسل بيانات عملاء حقيقية."),
    _s("decision.free_dev_profile", "decision", "ملف التطوير المجاني", False, type=TYPE_BOOL,
       help_ar="mock محلي + بيانات اصطناعية: صفر تكلفة، صفر شبكة، صفر مفاتيح."),
    _s("decision.note", "decision", "ملاحظة تشغيلية", "", type=TYPE_STR,
       help_ar="تظهر في سجلات القرار لتوضيح سبب تشغيل مزوّد معيّن."),
